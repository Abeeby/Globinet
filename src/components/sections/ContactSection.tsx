...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.previewUrl) {
        console.log('Prévisualisation email (développement):', data.previewUrl)
      }
    } catch (err: any) {
      setIsSubmitting(false)
      setSubmitError(err?.message || "Impossible d'envoyer votre message.")
    }
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 max-w-6xl mx-auto">
          ...
          >
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Success Message */}
              <motion.div
                className={`success-message ${isSubmitted ? '' : 'hidden'} text-center p-4 glass rounded-xl`}
                initial={{ opacity: 0 }}
              >
              ...
              {submitError && (
                <div className="text-center p-4 rounded-xl border border-red-500/30 bg-red-500/10">
                  <span className="text-red-400 font-sora">{submitError}</span>
                </div>
              )}
            </form>
          </motion.div>
